...
class StringGeneratorTest {
    ...
    fun random() {
        ...
        assertEquals("1", StringGenerator(faker)())
    }

    @Test
    fun constant() {
        assertEquals("2", StringGenerator().constant("2")())
    }

    @Test
    fun `format of valid pattern`() {
        val regex = "[azAZ]+"
        assertTrue { StringGenerator().format(regex)().matches(Regex(regex)) }
    }

    @Test
    fun `format of invalid pattern`() {
        assertThrows<IllegalArgumentException> { StringGenerator().format("a{-+1}")() }
    }
